...
$(document).ready(function(){
	...
	window.CHAT = {
		...
		logout:function(){
			// 发送退出登录指令
			CHAT.socket.send("[LOGOUT][" + new Date().getTime() +"][" + nickname + "][WebSocket]");
		...
		init:function(nickname){
			...
			// 给个默认头像
			var getPhoto = function(nickname){
			    var names = {'Big':'Big','Cover':'Cover','Tom':'Tom','Tony':'Tony'};
			    return names[nickname] || 'Tony';
            };
			...
			var appendToPanel = function(message){
				...
				if(cmd == "SYSTEM"){
			        ...
			        var photoUrl = '/images/photos/' + getPhoto((isme ? CHAT.nickname : name)) + '.jpg';
